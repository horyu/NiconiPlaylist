# 動画メタデータ取得・保存方針

本書は、NiconiPlaylist が動画タイトル、サムネイル、再生時間などのメタデータを取得する際の API 利用方針を整理する。

## 1. 目的

プレイリスト共有 URL の `videoIds` には動画 ID しか含まれない。

そのため、ポップアップやオプションページで以下を表示するには、別途動画メタデータの取得が必要になる。

- 動画タイトル
- サムネイル URL
- 再生時間
- 動画概要の一部

## 2. 使用 API

使用 API は以下。

```txt
GET https://nvapi.nicovideo.jp/v1/videos?watchIds={videoId}
```

リクエストヘッダは以下を付与する。

```txt
x-Frontend-Id: 6
x-Frontend-Version: 0
```

User-Agent の対応方針は [User-Agent 対応方針](./user-agent.md) を正とする。

## 3. 単体取得の実測結果

Node.js から以下のリクエストを実行し、`200 OK` を確認した。

```ts
await fetch("https://nvapi.nicovideo.jp/v1/videos?watchIds=so46209323", {
  method: "GET",
  headers: {
    "x-Frontend-Id": "6",
    "x-Frontend-Version": "0",
  },
});
```

- `credentials: "include"` は不要
- 単体の `watchIds` 指定では `200 OK` が返る
- JSON には `title`, `registeredAt`, `contentType`, `thumbnail`, `duration`, `shortDescription`, `count`, `owner`, `isChannelVideo`, `isPaymentRequired`, `requireSensitiveMasking` などが含まれる

```json
{
  "watchId": "so46209323",
  "video": {
    "id": "so46209323",
    "contentType": "long",
    "title": "名探偵プリキュア！ 第12話「キュアアルカナ・シャドウの秘密」",
    "registeredAt": "2026-04-26T09:00:00+09:00",
    "count": {
      "view": 130538,
      "comment": 38875,
      "mylist": 63,
      "like": 2568
    },
    "thumbnail": {
      "url": "...",
      "middleUrl": "...",
      "largeUrl": "...",
      "listingUrl": "...",
      "nHdUrl": "..."
    },
    "duration": 1450,
    "shortDescription": "...",
    "latestCommentSummary": "...",
    "isChannelVideo": true,
    "isPaymentRequired": true,
    "playbackPosition": null,
    "owner": {
      "ownerType": "channel",
      "id": "ch2650013",
      "name": "名探偵プリキュア！",
      "type": "channel",
      "iconUrl": "..."
    },
    "requireSensitiveMasking": false
  }
}
```

## 4. 複数取得について

`watchIds` の複数指定は、2026/04/29 時点では利用できない。

確認した結果:

- `?watchIds=so46209323&watchIds=sm25950409`
  - 最後の 1 件だけ取得される
- `?watchIds=so46209323,sm25950409`
  - `400 Bad Request`
- `?watchIds[]=so46209323&watchIds[]=sm25950409`
  - `500 Internal Server Error`
- `?watchIds[0]=so46209323&watchIds[1]=sm25950409`
  - `500 Internal Server Error`
- `?watchIds=["so46209323","sm25950409"]`
  - `400 INVALID_PARAMETER`

そのため、本プロジェクトではこの API を複数動画一括取得 API としては扱わない。

## 5. 利用方針

動画メタデータは都度描画のたびに取得しない。

方針は以下。

- 動画メタデータは background 側で取得する
- ポップアップやオプションページでは、表示対象プレイリストの `videoIds` を見て、未取得の動画だけを取得キューへ積む
- 取得したメタデータは拡張機能の local storage に保存する
- ポップアップ表示時は原則として local storage に保存済みのメタデータを参照する
- メタデータが未取得の動画のみを追加取得対象とする
- 一括取得が使えない前提で、必要な動画 ID を単体取得で順次または制限付き並列取得する
- 取得済みメタデータは再利用し、通常表示では再取得しない
- API の一時障害やメンテナンス中は、その動画 ID を再試行対象としてキューへ戻してよい
- プレイリスト単位 JSON のインポート時は、同梱された動画メタデータ / 投稿者情報も local storage に取り込んでよい
- プレイリスト単位 JSON のエクスポート時は、そのプレイリストが参照する動画メタデータ / 投稿者情報を同梱してよい

## 6. 保存戦略

動画メタデータは `watchId` 単位で保持する。

投稿者情報は動画メタデータに埋め込まず、`owner.id` を参照キーとして別に保持する。

動画メタデータは基本的に永続保持する。

どの保存済みプレイリストからも参照されない動画メタデータは、クリーンアップ対象として扱う。

クリーンアップは自動実行せず、ユーザーの明示操作でのみ行う。

投稿者情報も同様に基本的に永続保持し、どの動画メタデータからも参照されない場合にクリーンアップ対象とする。

## 7. 更新戦略

メタデータは、未取得動画の補完を目的として自動取得してよい。

一方で、取得済みメタデータの鮮度維持を目的とした定期再取得は行わない。

API 応答が `not_found` の場合は保存せず、その回の取得を完了扱いとしてよい。

HTTP `429` / `5xx`、network error、応答 JSON の読み取り失敗などの一時障害は、一定時間のクールダウン後に再試行してよい。

一時障害時も、失敗した 1 件だけを再試行対象として保持し、後続の取得キュー全体を恒久停止させない。

再取得は必要になった動画だけを対象にし、ユーザーの明示操作または新規参照時の不足補完に限る。

プレイリスト単位 JSON から取り込んだメタデータも保存済みデータとして扱い、通常表示のためだけに即時再取得しない。

## 8. 保存対象

動画 ID に付随して、以下を保持する。

- `watchId`
- `title`
- `registeredAt`
- `contentType`
- `thumbnail.url`
- `thumbnail.middleUrl`
- `thumbnail.largeUrl`
- `thumbnail.listingUrl`
- `thumbnail.nHdUrl`
- `duration`
- `isChannelVideo`
- `isPaymentRequired`
- `requireSensitiveMasking`
- `ownerId`
- `fetchedAt`

投稿者情報は別単位で以下を保持する。

- `owner.id`
- `owner.name`
- `owner.type`
- `owner.iconUrl`
- `fetchedAt`

以下は現時点では保存しない。

- `shortDescription`
- `count.view`
- `count.comment`
- `count.mylist`
- `count.like`
- `latestCommentSummary`
- `playbackPosition`
- `isMuted`
- `videoLive`
- API 応答中の意味が安定しない追加フラグ類

`registeredAt` / `contentType` / `isChannelVideo` / `isPaymentRequired` / `requireSensitiveMasking` は、動画の性質として比較的安定しているため保存対象に含めてよい。

`shortDescription` は将来的に更新されうるため、保存対象に含めない。

## 9. 表示戦略

ポップアップやオプションページは保存済みメタデータを表示する。

未取得の動画がある場合のみ追加取得する。

既存メタデータが存在する場合は、それを優先して表示する。

新しく追加・編集したプレイリストの動画や、ポップアップ / オプションページで初めて表示された動画も同じ方針で補完する。
