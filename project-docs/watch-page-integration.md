...
- 現在の動画 ID を再同期する
- 期待している動画 ID と実際の動画 ID を照合する
- route-ready 待ちを開始する

再生終了、リピート、次動画遷移、URL 変更時の判断は `src/lib/playbackTransition.ts` の純粋な状態遷移関数へ集約する。content script は、状態遷移関数が返した命令に従って runtime message、キーイベント、URL 変更などの副作用を実行する。
...
watch ページでは、拡張機能が次に維持・表示すべき動画を決めた時点で `expectedNextVideoId` を content script 側に保持する。

- 次動画へ進む場合は次動画 ID を設定する
- 現在動画をリピートする場合は現在動画 ID を設定する

その後 `niconiplaylist:locationchange` が発生した時は、

- 現在の `watch/{videoId}` と `expectedNextVideoId` を照合する
- 別の動画 ID へ遷移していた場合は、`location.href = buildWatchUrl(expectedNextVideoId)` で正しい URL へ戻す

この処理を入れている理由は以下。

- 拡張機能側が次動画遷移を実行した後に、ニコニコ動画公式の自動遷移が遅れて走り、別動画へ上書きされることがある
- 現在動画のリピート操作後にも、ニコニコ動画公式の自動遷移が走ることがある
...
現在動画のリピートは、ニコニコ動画側のプレイヤー状態と整合させるため、動画要素の `currentTime` や `play()` を直接変更せず、テンキーではない `0` キーの `keydown` イベントで先頭再生を要求する。

この操作よりニコニコ動画公式の自動遷移が優先された場合は、期待動画 ID との照合により現在動画へ強制復帰する。

強制復帰でフルリロードが発生すると content script のループ回数が失われるため、現在動画の完了済みループ回数を以下の条件で一時保持する。

- 強制復帰の直前に `sessionStorage` へ保存する
- 同じ動画へ戻った場合のみ復元する
- 保存から `10s` を過ぎたデータは復元しない
- 復元データは一度読み取った時点で削除する

これにより、想定外遷移から戻った後も、回数・時間指定リピートの進捗を継続する。
...
終了処理では、現在の再生状態と background が解決した次動画情報から、以下のいずれかを状態遷移関数で決定する。

- 現在動画をリピートする
- 解決済みの次動画へ進む
- playback context を終了し、必要なら完了通知を行う
